...
// (bug.0059). This fork's adapter logs all HTTP errors at warn (it has not taken
// the bug.5056 operator-fault split), so error-channel assertions use warn.
...
describe("LiteLlmAdapter", () => {
  ...
  describe("bug.0059: private error diagnostics in operator logs", () => {
    ...
    it("returns [unreadable] when response body cannot be read", async () => {
      ...
      // This fork's adapter logs all HTTP errors at warn (no bug.5056
      // operator-fault split) — assert warn, matching the adapter.
      expect(mockLoggerWarn).toHaveBeenCalledWith(
        expect.objectContaining({
